...
def print_result(cordinates, size):
    (row_index, col_index) = cordinates
    for r in range(row_index, row_index + size):
        row = []
...
matrix = read_matrix()
cordinates = get_best_submatrix_sum(matrix, SUBMATRIX_SIZE)
print_result(cordinates, SUBMATRIX_SIZE)